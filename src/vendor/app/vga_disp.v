...
assign VGA_VSYNC = vs;
assign VGA_HSYNC = hs;
// 选题二 扩展④ : 16x16 BMP 放大显示到 256x256 (每像素 16x16 屏幕点)
assign addr = {7'b0000000,y[7:4],x[7:4]};
...
always @(posedge clk25M or negedge reset_n) begin
	if(!reset_n)
		...
		if (hcnt < 640 && vcnt < 480 && dis_en)	begin	//扫描终止
			VGA_D[11:8] <= {rgb[7],rgb[6],2'b00};
			VGA_D[7:4]  <= {rgb[5],rgb[4],rgb[3],rgb[2]};
			VGA_D[3:0]  <= {rgb[1],rgb[0],2'b00};
		end
